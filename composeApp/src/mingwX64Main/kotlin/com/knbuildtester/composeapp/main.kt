...
fun main() {
    println("KN Build Tester - Native Application")
    println("=====================================")
    ...
    runBlocking {
        ...
        if (initResult.success) {
            ...
            // Run stress test
            println("\nRunning stress test...")
            val stressResult = facade.executeStressTest(100)
            ...
            println("\nRunning algorithm benchmark...")
            val algorithmResult = facade.executeComplexAlgorithms()
            println("Algorithm Benchmark Results:")
            algorithmResult.results.forEach { (name, time) ->
                println("  $name: ${"%.2f".format(time)} ms")
            }
            println("  Total Execution Time: ${"%.2f".format(algorithmResult.totalExecutionTime)} ms")
