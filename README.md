# KN + staticLib C interop on OHOS

Demo: a **cinterop klib** (with a C static lib), a **Kotlin/Native** Gradle project that produces a shared library, and a **C driver** that calls it. OHOS only.
...
| Part | Role |
|------|-----|
| **add** | Kotlin Multiplatform library. Defines a **cinterop** that wraps the C static lib `libadd.a` (built from `add.c`). Publishes the resulting klib to `maven-repo/` so the root can depend on it. |
