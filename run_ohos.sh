...
if ./gradlew -PpartialLinkMode=disable :app:clean :app:linkDebugSharedOhosArm64 --console=plain --rerun-tasks --refresh-dependencies; then
...
./gradlew -PpartialLinkMode=enable :app:clean :app:linkDebugSharedOhosArm64 --console=plain --rerun-tasks --refresh-dependencies

echo "👀 Compiling and running C caller (will crash on missing symbols)..."
clang -o runner c-caller/main.c -I app/build/bin/ohosArm64/debugShared -L app/build/bin/ohosArm64/debugShared -lapp -rpath app/build/bin/ohosArm64/debugShared
...
./gradlew -PpartialLinkMode=enable :app:linkDebugExecutableOhosArm64 --console=plain --rerun-tasks

echo "👀 Running standalone executable (will crash on missing symbols)..."
if ./app/build/bin/ohosArm64/debugExecutable/app.kexe; then
  echo "❌ Application should crash at runtime yet it didn't!"
else
