...
/**
 * @brief Sets the operating and maintenance options for model loading.
 *
 * This API is optional and takes effect during model loading. It is used to debug model performance.
 *
 * @param compilation Pointer to {@link OH_NNCompilation}. The value cannot be null. Otherwise, an error code will
 * be returned.
 * @param type Operating and maintenance type {@link HiAI_OmType}.
 * @param outputDir Output directory used to store Operating and maintenance files.
 * @return Function execution result. If the operation is successful, OH_NN_SUCCESS is returned. If the operation fails,
